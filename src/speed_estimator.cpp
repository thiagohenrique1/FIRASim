...
speedEstimator::estimateSpeed(int time, dReal *pose, dReal *vel){
    time = time / 1000;
    int scale = 1;
    if(this->prev_time==0 || (time - this->prev_time) == 0){
        ...
        double vel_yaw;
        if (this->have_angle){
           vel_yaw = -smallest_angle_diff(pose.yaw, self.prev_pos[2]) / dt; //Implementar depois essa função
        }     
        double lin = sqrt(vel_x ** 2 + vel_y ** 2);
        ...
        this->corr_moving_avg_x = this->moving_avg_x / (1 - this->beta ** this->count_avg);
        this->moving_avg_y = this->beta * this->moving_avg_y + (1 - this->beta) * vel_y;
        this->corr_moving_avg_y = this->moving_avg_y / (1 - this->beta ** this->count_avg);
        if (this->have_angle){
            this->moving_avg_yaw = this->beta * this->moving_avg_yaw + (1 - this->beta) * vel_yaw;
            this->corr_moving_avg_yaw = this->moving_avg_yaw / (1 - this->beta ** this->count_avg);
            this->prev_pos[0] = pose[0];
            this->prev_pos[1] = pose[1];
            this->prev_pos[2] = pose[2];   
